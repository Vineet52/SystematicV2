...
let buildTable=function(tmp,assArr,assPArr)
{
	...
	let formEntry="<form action='finalise_delivery.php' method='GET'><input type='hidden' name='ass' value='"+JSON.stringify(saleArr)+"'>"+"<input type='hidden' name='assP' value='"+JSON.stringify(saleProductsArr)+"'>"+"<input type='hidden' name='address' value='"+saleArr[0]["ADDRESS_NAME"]+"'>"+"<button class='btn btn-icon btn-2 btn-success btn-sm' type='submit'><span class='btn-inner--icon'><i class='fas fa-truck'></i></span><span class='btn-inner--text'>Make Delivery</span></button>";
